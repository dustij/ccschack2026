...
export function ContributorsMarquee() {
  return (
    ...
    >
      ...
      {/* marquee wrapper */}
      <div
        className="contributors-marquee relative overflow-hidden"
        /* fade-out edges */
        style={{
          maskImage:
      ...
      >
        <div
          className="contributors-track flex w-max whitespace-nowrap"
          style={{ animation: 'marquee-rtl 110s linear infinite' }}
        >
          {[0, 1].map((dupIndex) => (
            <div key={dupIndex} className="flex shrink-0 gap-6 pr-6">
              {CONTRIBUTORS.map((c) => (
                <div
                  key={`${dupIndex}-${c.name}`}
                ...
                >
                  {/* Avatar initial */}
                  <span className="flex h-8 w-8 items-center justify-center rounded-full bg-linear-to-br from-purple-500 to-pink-500 text-xs font-extrabold text-white shadow-lg">
                    {c.name.charAt(0)}
                  </span>
      ...
      <style>{`
        ...
        .contributors-marquee:hover .contributors-track {
